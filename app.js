...
const SUPPORTED_LANGUAGES = ["uz", "en", "ru", "ko"];
...
const DEMO_PRODUCTS = [
  {
    ...
    todayDeal: true,
    favorite: false,
    imageUrl: CONFIG.placeholderImage,
    variants: [{ id: "demo-variant-1", label: "30 ml", price: 99000, discountPrice: 80000, stock: 8 }],
  },
  {
    id: "demo-2",
    name: "Lip & Cheek Balm",
    ...
    favorite: false,
    imageUrl: CONFIG.placeholderImage,
    variants: [{ id: "demo-variant-2", label: "Rose", price: 150000, discountPrice: 110000, stock: 4 }],
  },
];
...
const els = {
  ...
  homeApiSections: document.getElementById("homeApiSections"),
  ...
  closeCatalog: document.getElementById("closeCatalog"),
  catalogList: document.getElementById("catalogList"),
  status: document.getElementById("productStatus"),
  dealsStatus: document.getElementById("dealsStatus"),
  productsMode: document.getElementById("productsMode"),
  dealsMode: document.getElementById("dealsMode"),
  title: document.getElementById("productsTitle"),
  ...
  productDetailPageContent: document.getElementById("productDetailPageContent"),
  ...
  myReviewsDialog: document.getElementById("myReviewsDialog"),
  myReviewsContent: document.getElementById("myReviewsContent"),
  myReviewsSummary: document.getElementById("myReviewsSummary"),
  refreshMyReviews: document.getElementById("refreshMyReviews"),
  closeMyReviews: document.getElementById("closeMyReviews"),
  writeReviewDialog: document.getElementById("writeReviewDialog"),
  writeReviewForm: document.getElementById("writeReviewForm"),
  closeWriteReview: document.getElementById("closeWriteReview"),
  writeReviewProduct: document.getElementById("writeReviewProduct"),
  reviewRatingSelector: document.getElementById("reviewRatingSelector"),
  reviewContent: document.getElementById("reviewContent"),
  reviewImages: document.getElementById("reviewImages"),
  reviewImageFiles: document.getElementById("reviewImageFiles"),
  reviewUploadStatus: document.getElementById("reviewUploadStatus"),
...
function normalizeSavedBaseUrl(value) {
...
function normalizeProduct(product = {}) {
  ...
  const selectedVariant = variants.find((variant) => Number(variant.stock || 0) > 0) || variants[0] || null;
  const images = normalizeImages(product.images);
  const detailImages = normalizeImages(product.detailImages);
  const mainImage = imageValue(product.mainImageUrl) || images[0] || product.imageUrl || CONFIG.placeholderImage;
  const originalPrice = numberOrZero(product.price ?? selectedVariant?.price);
  const finalPrice = numberOrZero(selectedVariant?.discountPrice ?? product.discountPrice ?? product.price);
...
function setFavoriteProducts(products) {
  state.favoriteProducts = products.filter((product) => product.id !== undefined && product.id !== null);
...
function syncProductFavorites() {
  state.products = state.products.map((product) => ({ ...product, favorite: state.favoriteIds.has(String(product.id)) }));
  state.todayDeals = state.todayDeals.map((product) => ({ ...product, favorite: state.favoriteIds.has(String(product.id)) }));
...
function t(key, params = {}) {
  ...
  const fallback = translations.en || translations[DEFAULT_LANGUAGE];
  const template = dictionary?.[key] ?? fallback?.[key] ?? translations[DEFAULT_LANGUAGE]?.[key] ?? key;
...
function getNotificationsContent(response) {
...
function normalizeMyReviewItem(item = {}) {
...
function getMyReviewsContent(response) {
  if (Array.isArray(response?.data)) return response.data;
  if (Array.isArray(response?.data?.content)) return response.data.content;
